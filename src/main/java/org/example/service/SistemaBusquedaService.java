...
public class SistemaBusquedaService {
    ...
    private void procesarDirectorio(Directorio directorio) {
        ...
        for (Archivo docActual : archivosEncontrados) {
            ...
            if (necesitaIndexacion) {
                ...
                indexador.indexarArchivo(
                        docActual.getIdArchivo(),
                        docActual.getNombreArchivo(),
                        textoPlano
                );
            }
        }
    ...
    public List<Archivo> realizarBusqueda(String consultaUsuario) {
        return realizarBusqueda(consultaUsuario, null);
    }

    /**
     * Ejecuta una búsqueda textual con filtro opcional por tipo de archivo.
     */
    public List<Archivo> realizarBusqueda(String consultaUsuario, String tipoArchivo) {
        logger.info("Ejecutando búsqueda: '{}' (filtro: {})", consultaUsuario, tipoArchivo);

        List<Archivo> archivosResultado = new ArrayList<>();

        List<CoincidenciaIndice> coincidencias = indexador.buscar(consultaUsuario, 50);
        ...
        String registroConsulta = tipoArchivo != null && !tipoArchivo.isBlank()
                ? consultaUsuario + " [tipo:" + tipoArchivo + "]"
                : consultaUsuario;
        registroBusqueda.setFiltrosAplicados(registroConsulta);
        ...
        for (CoincidenciaIndice coincidencia : coincidencias) {
            int idArchivo = coincidencia.getIdArchivo();
            float score = coincidencia.getScore();

            Optional<Archivo> match = archivoDAO.buscarPorId(idArchivo);

            if (match.isPresent()) {
                Archivo archivo = match.get();
                if (tipoArchivo != null && !tipoArchivo.isBlank()
                        && !tipoArchivo.equalsIgnoreCase(archivo.getTipoArchivo())) {
                    continue;
                }

                archivosResultado.add(archivo);
    ...
    /**
     * Obtiene métricas básicas del sistema para el panel de estado.
     */
    public EstadisticasSistema obtenerEstadisticas() {
        List<Directorio> directorios = directorioDAO.obtenerTodos();
        int activos = (int) directorios.stream()
                .filter(d -> "activo".equalsIgnoreCase(d.getEstado()))
                .count();
        ...
        return new EstadisticasSistema(
                ...
                activos,
                directorios.size(),
                archivoDAO.obtenerUltimaModificacion()
        );
    }

    /**
     * Lista directorios con la cantidad de archivos indexados por cada uno.
     */
    public List<DirectorioVista> obtenerDirectoriosIndexados() {
        List<DirectorioVista> vistas = new ArrayList<>();
        for (Directorio dir : directorioDAO.obtenerTodos()) {
            int total = archivoDAO.contarPorDirectorio(dir.getIdDirectorio());
            vistas.add(new DirectorioVista(dir, total));
        }
        return vistas;
    }
}
